...
testingApp.controller('AppointmentRequestListCtrl', function ($scope, $http) {
    ...
    $scope.reload = function () {
        if ($scope.doReload) {
            $http.get('http://api.catcal.zone/appointmentRequest').success(function (data, status, headers, config) {
...
testingApp.controller('AppointmentRequestCtrl', function ($scope, $http) {
    ...
    $scope.createNewAppointmentRequest = function (appointmentRequest, form) {
        // 52651
        var url = 'http://api.catcal.zone/appointmentRequest'; // TODO add profiles somehow to determin right url
